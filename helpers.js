...
export const findUserFromPic = (picId, term, headers) => {
	let tmpHeadersDict = Object.assign({}, headers, {'accept': 'application/json', 'content-type': 'application/json'})
	...
	return new Promise((resolve, reject)=> {
		...
		.end((err, res)=> {
			if(!!err) {
				...
				resolve({id:res.body.media.owner.id, username:res.body.media.owner.username})
			}
		})
	})
}
